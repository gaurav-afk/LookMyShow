...
""".trimIndent()
    ...
    fun getMockTheaters(): TheaterListResponse? {
        ...
        Log.d("MockTheaterDataSource", "Attempting to parse mock theater JSON") // Log
        return try {
            val parsed = adapter.fromJson(mockJsonResponse) // your theater JSON string
            Log.d("MockTheaterDataSource", "Parsed theaters count: ${parsed?.theaters?.size}") // Log
            parsed
        } catch (e: Exception) {
            Log.e("MockTheaterDataSource", "Error parsing theater JSON", e) // Log error
            null
        }
    }
}